...
def load_trapnell(data_path="/s/project/poisson_atac/anndata/", convert_counts=True):
    cache_path = os.path.join(data_path, "all_tissues_filtered.h5ad")
...
def load_aerts(
    ...
    if cached:
        ...
        if filter_unknown:
            adata = adata[
                (~adata.obs.CellType_lvl1.str.contains("unk"))
...
def load_neurips(
    ...
    path = os.path.join(
        data_path,
        "openproblems_bmmc_multiome_phase2.manual_formatting.output_mod2.h5ad",
    )
    adata = ad.read(path)
    ...
    if gex and not multiome:
        path = os.path.join(
            data_path,
            "openproblems_bmmc_multiome_phase2.manual_formatting.output_rna.h5ad",
        )
...
def load_hematopoiesis(
    data_path="/s/project/poisson_atac/anndata/", convert_counts=True
):
    cache_path = os.path.join(data_path, "GSE129785_scATAC-Hematopoiesis.h5ad")
    cached = os.path.exists(cache_path)
    if cached:
        adata = ad.read(cache_path)
        adata.obs_names_make_unique()
        ...
        fn = [
            os.path.join(
                data_path,
                "GSE129785_scATAC-Hematopoiesis",
                "GSE129785_scATAC-Hematopoiesis-All.mtx",
            ),
            os.path.join(
                data_path,
                "GSE129785_scATAC-Hematopoiesis",
...
def save_for_seurat(adata, outdir, sep=[("-", ":")]):
    ...
    for replacement in sep:
        peaks.loc[:, "peaks"] = peaks.loc[:, "peaks"].str.replace(
    ...
    adata.obs.to_csv(os.path.join(outdir, "metadata.csv"), index=False)
