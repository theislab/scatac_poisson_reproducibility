...
class MULTIVAE(BaseModuleClass):
    ...
    gene_likelihood
        ...
        * ``'poisson'`` - Poisson
    peak_likelihood
        The distribution to use for chromatin accessibility data. One of the following
        * ``'bernoulli'`` - Bernoulli, binarized data
        * ``'poisson'`` - Poisson
    n_hidden
    ...
    use_fragment_count_key
        Use fragment_count AnnDataField defined by the user as scaling factor in mean of conditional ATAC distribution.
    """
    ...
    def __init__(
        ...
        use_fragment_count_key: bool = True
    ):
        super().__init__()
        ...
        self.use_fragment_count_key = use_fragment_count_key
        ...
        ##TODO: check that we use same encode as in poissonvae
        ...
        #TODO: add option to use different Decoders based on peak likelihood
        # accessibility decoder
        self.z_decoder_accessibility = DecoderPoissonVI(
    ...
    def inference(
        ...
        if self.peak_likelihood == "bernoulli":
            ...
            libsize_acc = torch.log(x_chr.sum(axis=1)).unsqueeze(1)
    ...
    def get_reconstruction_loss_accessibility(self, x, p):
        if self.peak_likelihood == "bernoulli":
