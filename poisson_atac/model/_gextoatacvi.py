...
from poisson_atac.module import GEXTOATACVAE, BinaryGEXTOATACVAE
from torch.distributions import Poisson
class GEXtoATAC(ArchesMixin, VAEMixin, UnsupervisedTrainingMixin, BaseModelClass):
    """
    ...
    **model_kwargs
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        print(len(self.adata_manager.get_state_registry("GEX")['column_names']))
        n_genes = len(self.adata_manager.get_state_registry("GEX")['column_names'])

        n_batch = self.summary_stats.n_batch

        self.module = GEXTOATACVAE(
    ...
    def get_imputet_accessibility(self,
        ...
        for tensors in post:
            ...
            px_rate = generative_outputs["px_rate"].cpu()
    ...
    def get_normalized_accessibility(self, adata, batch_size = 128, use_z_mean=True, library=1):
        adata = self._validate_anndata(adata)
        indices = np.arange(adata.n_obs)
        ...
        for tensors in post:
            ...
            px_scale = generative_outputs["px_scale"].cpu()
            p = 1 - torch.exp(Poisson(library*px_scale).log_prob(torch.Tensor([0])))
            imputed.append(p)

        imputed = torch.cat(imputed).detach().numpy()
        return imputed
    ...
    def setup_anndata(
        cls,
        adata: AnnData,
        adata_gex_obsm_key: str,
        adata_gex_names_uns_key: Optional[str] = None,
        ...
        **kwargs,
        ...
        anndata_fields = [
            ...
            ObsmField("GEX", 
                      adata_gex_obsm_key, 
                      colnames_uns_key=adata_gex_names_uns_key,
                      is_count_data=True)
        ]
...
class BinaryGEXtoATAC(ArchesMixin, VAEMixin, UnsupervisedTrainingMixin, BaseModelClass):
    """
    Peak Variational Inference [Ashuach21]_
    ...
    **model_kwargs
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        n_genes=len(self.adata_manager.get_state_registry("GEX")['column_names'])
        self.module = BinaryGEXTOATACVAE(
            n_input_regions=self.summary_stats.n_vars,
            n_input_genes=n_genes,
            n_batch=self.summary_stats.n_batch,
            n_hidden=n_hidden,
