...
class ExperimentWrapper:
    ...
    def init_dataset(self, dataset, batch, downsample):
        """
        ...
        if dataset == "neurips":
            self.adata = patac.data.load_neurips(data_path='/s/project/poisson_atac/anndata', batch=batch, only_train=False)
        
        self.adata.X = self.adata.layers['counts'] #copy counts to X layer
        total_counts = self.adata.X.sum() #determine total counts
        sc.pp.downsample_counts(self.adata, total_counts=total_counts*downsample) #downsample adata.X

        self.adata.layers['counts'] = self.adata.X #copy back to counts layer
        self.adata.X = (self.adata.X > 0).astype(float) #binarize X
    ...
    def init_model(self, model_type: str):
        ...
        if model_type == "peakvi":
            ...
            self.model = scvi.model.POISSONVI
